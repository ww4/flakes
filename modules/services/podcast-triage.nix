...
in
{
  ...
  config = lib.mkIf cfg.enable {
    # Fail at EVAL, not at 06:00 on a Thursday. With no archives the ranker
    # prints usage and exits 2, the unit's own guard swallows that as "ranking
    # failed" and exits 0 — a green weekly run that triaged nothing, forever.
    # The whole point of this module is that silence must not read as "nothing
    # worth recommending".
    assertions = [{
      assertion = cfg.archives != [ ];
      message = "services.podcastTriage.enable is true but archives is empty — the weekly run would report success having triaged nothing.";
    }];
    ...
    systemd.services.podcast-triage = {
      ...
      serviceConfig = {
        ...
        Environment = [
          ...
          # ⚠️ The archive list is NOT passed here. It used to be a
          # TRIAGE_ARCHIVES entry holding the paths joined by spaces —
          # and systemd splits `Environment=` on whitespace, so only the FIRST
          # archive would ever have been triaged while the unit exited 0. That
          # is not hypothetical: the sibling content-archives module shipped the
          # identical bug and refreshed one of eight archives for three weeks
          # (flakes #264). It is argv below instead; podcast-triage.py already
          # accepts positional archive paths.
        ];
      ...
      script = ''
        ...
        if ! ${lib.getExe triage} --limit ${toString cfg.limit} ${lib.escapeShellArgs cfg.archives} > "$out"; then
