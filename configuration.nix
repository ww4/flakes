...
# FILESYSTEMS
 ...
 environment.systemPackages = with pkgs; [
  ...
  services = {
    ...
    # Bitcoin Core — runs as chris against the existing ~/.bitcoin (pruned
    # node, ~17 GB). prune is also recorded in ~/.bitcoin/settings.json, which
    # was stripped of that key so this is the single source of truth.
    bitcoind.bitcoin = {
      enable = true;
      user = "chris";
      group = "users";
